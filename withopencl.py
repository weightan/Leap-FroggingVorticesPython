...
List_Strengh_Vortexes = np.array([1, -1, 1, -1], dtype = np.float32)
List_of_Vort = np.array([-1.0 - 0.5j, -1.0 + 0.5j, -0.5 - 0.5j, -0.5 + 0.5j], dtype=np.complex128)
...
def calc_frame_opencl(q, vort, vortstrength = List_Strengh_Vortexes):
    ...
    ///constant  double dt = 0.1;
    #define vortN 4
    
    double2 fValue( int K, double2 Z, double2 Z0){
        double r2 = pow(   sqrt( pow((Z.x - Z0.x), 2) +  pow((Z.y - Z0.y), 2)  ),      2);
        double constM =  K*(1 - exp((-r2)/(rcore*rcore))) / r2  ;
